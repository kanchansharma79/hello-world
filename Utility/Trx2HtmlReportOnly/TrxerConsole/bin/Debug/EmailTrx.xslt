...
>
  ...
  <msxsl:script language="C#" implements-prefix="trxreport">
    ...
    public static string GenerateClassName(string asm)
    {
        ...
        <div id="divToRefresh" class="wrapOverall">
          ...
          <div class="StatusBar statusBar{$testRunOutcome}">
            <div class="statusBar{$testRunOutcome}Inner">
              <center>
                <h1 class="hWhite">
                  <div class="titleCenterd">Test automation run result :- <xsl:value-of select="trxreport:GetShortDateString(/t:TestRun/t:Times/@creation)"/></div>
                ...
                <tr>
                  ...
                  <td class="ex">
                    <div class="OpenMoreButton" onclick="ShowHide('{generate-id(@className)}TestsContainer','{generate-id(@className)}Button','Show Tests','Hide Tests');">
                      <a id ="dashboard" href="kanchan.sharma@outlook.com" target="_blank">click here!!</a>
                    </div>
                  </td>
                </tr>
                  ...
                  <td colspan="5">
                    ...
                    <table>-->
                      <!--<thead>
                        <tr class="odd">
                          <th scope="col" class="TestsTable">Time</th>
                          <th scope="col" class="TestsTable" abbr="Status">TestID</th>
                          <th scope="col" class="TestsTable" abbr="Status">Status</th>
                          <th scope="col" class="TestsTable" abbr="Test">Test Name</th>
                          <th scope="col" class="TestsTable" abbr="Message">Computer</th>
  ...
  <xsl:template name="debugInfo">
    ...
    <xsl:for-each select="/t:TestRun/t:Results/t:UnitTestResult[@testId=$testId]/t:Output">
      ...
      <xsl:if test="$StdOut">
        <a style="float:right;" id="{generate-id($testId)}StdOutToggle" href="javascript:ShowHide('{generate-id($testId)}StdOut','{generate-id($testId)}StdOutToggle','Show Output','Hide Output');">Show Output</a>
      </xsl:if>
    </xsl:for-each>
  </xsl:template>
</xsl:stylesheet>