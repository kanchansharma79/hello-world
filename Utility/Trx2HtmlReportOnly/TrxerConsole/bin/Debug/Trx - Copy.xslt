...
>
          ...
          <table id="ReportsTable">
            ...
            <tbody>
              <xsl:for-each select="$classSet">
                ...
                <tr>
                  <th scope="row" class="column1"><xsl:value-of select="trxreport:GetShortDateString(/t:TestRun/t:Times/@creation)"/></th>                  
                  <td class="PackageStatus"> <canvas id="{generate-id(@className)}canvas" width="100" height="25"> </canvas> </td>
                  <td class="Function"> <xsl:value-of select="trxreport:GenerateClassName(@className)" /> </td>
                  <td class="Message" name="{generate-id(@className)}Id"><xsl:value-of select="concat($testsCount,' Tests')" /> </td>
                  <td class="ex">
          ...
          <Table>
            ...
            <thead>
              <tr class="odd">
                ...
                <th scope="col" abbr="TID">TestID</th>
                <th scope="col" abbr="Test">Test Name</th>
                <th scope="col" abbr="Status">Status</th>
                <th scope="col" abbr="Machine">Computer</th>
                <th scope="col" abbr="Time">Duration</th>
              </tr>
            </thead>
            <tbody>
  ...
  <xsl:template name="tDetails">
    ...
    <xsl:for-each select="/t:TestRun/t:Results/t:UnitTestResult[@testId=$testId]">
      ...
      <tr id="{generate-id($testId)}MessageErrorInfo" class="hiddenRow">
        <!--Outer-->
        <td colspan="6">
          <div id="exceptionArrow">↳</div>
          <table>
      ...
      <tr id="{generate-id($testId)}ResultFiles" class="hiddenRow">
        <!--Outer-->
        <td colspan="6">
          <div id="exceptionArrow">↳</div>
          <table>
  ...
  <xsl:template name="debugInfo">
    ...
    <xsl:for-each select="/t:TestRun/t:Results/t:UnitTestResult[@testId=$testId]/t:Output">
      ...
      <xsl:if test="$MessageErrorStacktrace">
        <a style="float:right;" id="{generate-id($testId)}StacktraceToggle" href="javascript:ShowHide('{generate-id($testId)}Stacktrace','{generate-id($testId)}StacktraceToggle','Stacktrace','Stacktrace');">Stacktrace</a>
      </xsl:if>
      
      <xsl:variable name="MessageErrorInfo" select="t:ErrorInfo/t:Message"/>
      <xsl:if test="$MessageErrorInfo">
        <a style="float:right;" id="{generate-id($testId)}MessageErrorInfoToggle" href="javascript:ShowHide('{generate-id($testId)}MessageErrorInfo','{generate-id($testId)}MessageErrorInfoToggle','Error Msg','Hide Msg');">Error Msg</a>
      </xsl:if>
      ...
      <xsl:for-each select="/t:TestRun/t:Results/t:UnitTestResult[@testId=$testId]/t:ResultFiles">
        ...
        <xsl:if test="$ResultFiles">
          <a style="float:right;" id="{generate-id($testId)}ResultFileInfoToggle" href="javascript:ShowHide('{generate-id($testId)}ResultFiles','{generate-id($testId)}ResultFileInfoToggle','Files','Hide Files');">Files</a>
        </xsl:if>
      </xsl:for-each>
      <!--<xsl:variable name="MessageErrorInfo" select="t:ErrorInfo/t:Message"/>
      <xsl:if test="$MessageErrorInfo">
